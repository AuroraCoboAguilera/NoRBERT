...
Implementation of Top NoRBERTA (Noisy Regulagarized ROBERTA), which applies a GM-VAE between the Encoder and the output layer
from ROBERTA, and Deep NoRBERTA, which applies a GM-VAE between the Encoder layers from ROBERTA.
It substitutes the files NoROBERTA and DeepNoROBERTA due to library updates.
...
from transformers.models.roberta.modeling_roberta import RobertaPreTrainedModel, RobertaModel, RobertaLMHead, RobertaEmbeddings, RobertaPooler, RobertaAttention, RobertaIntermediate, RobertaOutput
...
# ACA
class TopNoRoBERTa(RobertaPreTrainedModel):
    _keys_to_ignore_on_save = [r"lm_head.decoder.weight", r"lm_head.decoder.bias"]
    _keys_to_ignore_on_load_missing = [r"position_ids", r"lm_head.decoder.weight", r"lm_head.decoder.bias"]
    _keys_to_ignore_on_load_unexpected = [r"pooler"]
    def __init__(self, config, freeze_bert_encoder=True, GMVAE_model=None, K_select=-1):
        '''
                Class that define the TopNoRoBERTa model as the concatenation of RoBERTa (Transformer Encoder) with a classification
        ...
        print('NoRoBERTa initialized with freezed RoBERTa parameters: {}; and a GMVAE model: {}'.format(freeze_bert_encoder, (GMVAE_model != None)))
    ...
    def forward(
        ...
        if self.gmvae != None:
            ...
            prediction_scores = self.lm_head(sequence_output_GMVAE)
        else:
...
# ACA
class DeepNoRoBERTa(RobertaPreTrainedModel):
    _keys_to_ignore_on_save = [r"lm_head.decoder.weight", r"lm_head.decoder.bias"]
    _keys_to_ignore_on_load_missing = [r"position_ids", r"lm_head.decoder.weight", r"lm_head.decoder.bias"]
    _keys_to_ignore_on_load_unexpected = [r"pooler"]
    def __init__(self, config, freeze_bert_lowerEncoder=True, GMVAE_model=None, save_deep_hidden_state=False, layer_deep=None, double_flag=False, K_select=-1):
        '''
                Class that define the DeepNoRoBERTa model as the concatenation of RoBERTa (Transformer Encoder) with a classification
                layer to compute the [MASK] tokens and build the reconstruction of the sentence.
                It is based on the classes of the library transformers from Hugging face.
                Parameters:
                    - config: Is a class from the library transformers.
                    - freeze_roberta_lowerEncoder: Flag to freeze the parameters of RoBERTa lower than the position where the GMVAE is applied,
        ...
        self.roberta = RobertaModel_(config, GMVAE_model, save_deep_hidden_state, layer_deep, double_flag, add_pooling_layer=False, K_select=K_select) #ACA
        self.lm_head = RobertaLMHead(config)
            ...
            if double_flag == False:
                if layer_deep != None:
                    for nlayer in range(layer_deep-1):
                        for p in self.roberta.encoder.layer[nlayer].parameters():
                            p.requires_grad = False

                    for p in self.roberta.encoder.layer[layer_deep-1].attention.parameters():
                        p.requires_grad = False
                else:
                    ...
                    for p in self.roberta.encoder.layer[math.ceil(config.num_hidden_layers / 2)-1].attention.parameters():
                        p.requires_grad = False
            else:
                if len(layer_deep) != 2:
                    raise ValueError("You should select 2 layers to apply a DGM in Double Deep NoRBoERTa")
                else:
                    ...
                    for p in self.roberta.encoder.layer[freezer - 1].attention.parameters():
                        p.requires_grad = False


        if layer_deep != None:
            print('Customized {} Layer Deep NoRoBERTa initialized with freezed lower roberta parameters: {}; and a GMVAE model: {}'.format(layer_deep, freeze_bert_lowerEncoder, (GMVAE_model != None)))
        else:
            print('1/2 Deep NoRoBERTa initialized with freezed lower roberta parameters: {}; and a GMVAE model: {}'.format(freeze_bert_lowerEncoder, (GMVAE_model != None)))
...
class RobertaModel_(RobertaPreTrainedModel):
    ...
    def __init__(self, config, GMVAE_model, save_deep_hidden_state, layer_deep, double_flag, add_pooling_layer=True, K_select=-1):       # ACA
        ...
        self.encoder = RobertaEncoder(config, GMVAE_model, save_deep_hidden_state, layer_deep, double_flag, K_select=K_select)            # ACA
...
class RobertaEncoder(nn.Module):
        ...
        if double_flag:
            ...
            for nlayer in range(config.num_hidden_layers):
                if nlayer + 1 == layer_deep[0]:
                    layers.append(RobertaLayer(config, GMVAE_model[0], save_deep_hidden_state, K_select=K_select))
                elif nlayer + 1 == layer_deep[1]:
                    layers.append(RobertaLayer(config, GMVAE_model[1], save_deep_hidden_state, K_select=K_select))
                else:
                    layers.append(RobertaLayer(config, None))
            self.layer = nn.ModuleList(layers)
        else:
            if layer_deep == None:
                self.layer = nn.ModuleList([RobertaLayer(config, GMVAE_model, save_deep_hidden_state, K_select=K_select) if nlayer + 1 == math.ceil(config.num_hidden_layers / 2) else RobertaLayer(config, None) for nlayer in range(config.num_hidden_layers)])
            else:
                self.layer = nn.ModuleList([RobertaLayer(config, GMVAE_model, save_deep_hidden_state, K_select=K_select) if nlayer+1 == layer_deep else RobertaLayer(config, None) for nlayer in range(config.num_hidden_layers)])
    ...
    def forward(
        ...
        return BaseModelOutputWithPastAndCrossAttentions2(
            ...
            deep_hidden_state=deep_hidden_state,            # ACA
        )
        ...
        if self.gmvae != None:
            ...
            if self.gmvae.cuda:
                ...
                attention_output_GMVAE[(attention_mask == 0)[:, 0, 0, :]] = torch.Tensor(
                    self.gmvae.batch_reconstruction(att_out, K_select=self.K_select)).cuda()
            else:
                attention_output_GMVAE[(attention_mask == 0)[:, 0, 0, :]] = torch.Tensor(

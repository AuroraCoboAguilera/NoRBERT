...
If we have contextual length different to zero, it means that the input size is the embedding size * (contextual_length+1+contextual_length).
Then, the x dimension from inference network is different from the x dimension of the generative network (embedding size).
The ELBO must be modified so in the loglikelihood we only take the middle part from the x input with the contextual embeddings.
...
class GMVAE:
    def __init__(self, args, seq2seq=False):
        ...
        if hasattr(args, 'weight_cond'):
            self.weight_cond = args.weight_cond
        else:
            self.weight_cond = 1.0

        if hasattr(args, "contextual_length"):
            self.contextual_length = args.contextual_length
        else:
            self.contextual_length = 0

        self.reconstruction_dim = int(self.input_dim/(self.contextual_length*2+1))

        print('contextual_length: ' + str(self.contextual_length))
        print('reconstruction_dim: ' + str(self.reconstruction_dim))


        # Create the generative network
        self.generative = GenerativeNet(self.reconstruction_dim, self.z_dim, self.w_dim, self.K, self.hidden_dim, self.dropout,
    ...
    def train(self, train_loader, val_loader, bunch_n=0):
        ...
        for epoch in range(1, self.epochs + 1):
            ...
            if self.summary:
                self.write_to_tensorboard(writer, self.global_step, train_total_loss, train_log_lik, self.weight_cond * train_cond_loss,
                                          train_w_loss, train_y_loss, val_total_loss, val_log_lik, self.weight_cond * val_cond_loss,
    ...
    def ELBO(self, x):
        ...
        # ELBO regularizer: If we have contectual information, we apply the loglikelihod only to the middle token embedding
        self.log_lik = log_lik(self.sigma, x[:, self.contextual_length*self.reconstruction_dim:self.contextual_length*self.reconstruction_dim+self.reconstruction_dim], self.generative.x_z_mean)
        ...
        self.ELBO_loss = -(self.log_lik - self.weight_cond * self.cond_prior_l - self.w_prior_l - self.y_prior_l)
    ...
    def batch_reconstruction(self, x, K_select=-1):
        """Reconstruct Data

		Args:
			x: (tensor: batch_size * input_dim) data to be reconstructed
			K: (int) index of the component we use to reconstruct the sample. If -1, we do not use any of them (the reconstruction does no pass throught those steps)
		Returns:
			reconstructed: (array) array containing the reconstructed data
		"""
        self.inference.eval()
        ...
        if K_select == -1:
            ...
            self.generative.forward(self.inference.z_x_sample, w=self.inference.w_x_sample)
            reconstructed = self.generative.x_z_sample_stack[K_select].data.cpu().numpy()

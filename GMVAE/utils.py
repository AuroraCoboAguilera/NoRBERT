...
def get_args():
    ...
    parser.add_argument('--name_extension', type=str, default='2020-01-26 10:59:52',
                        help='Pattern to be found to obtain the data. Used by main.py (seq2seq models) (default: 2020-01-26 10:59:52)')
    parser.add_argument('--dataset_name', type=str, default='dataset1',
                        help='Dataset used (from the LM) for obtaining the data (default: dataset1)')
    parser.add_argument('--dataroot', type=str, default='./data/4_decoder_context/',
                        help='Root to load dataset. Used by main.py (seq2seq models) (default: ./data/3_decoder_output_enc/)')
    ...
    parser.add_argument('--reconstruct', type=int, default=0, help='Flag to get reconstructions (default: 0)')
    parser.add_argument('--K_select', type=int, default=-1, help='K to be selected in the reconstruction (default: -1)')
    parser.add_argument('--weight_cond', type=float, default=1.0, help='weight to give to the cond prior term in the loss function (default: 1)')
    parser.add_argument('--contextual_length', type=int, default=0, help='Number of token embeddings before and after the current token (default: 0)')
...
def get_model_name(config):
    ...
    if config.weight_cond != 1:
        model_name = model_name + '_' + str(config.weight_cond).replace('.', '_')
    return model_name
...
def get_config_and_flags(args):
    ...
    flags.reconstruct = args['reconstruct']
